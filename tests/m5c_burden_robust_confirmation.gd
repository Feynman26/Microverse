...
func _run() -> void:
	...
	for seed in range(25001, 25025):
		...
		if value > 0.0:
			positive += 1
		if bool(result["stable_both_divided"]):
			stable_valid += 1
		fluctuating_divisions += int(result["fluctuating_divisions"])
		print("M5-C BURDEN-ROBUST seed=%d Dnorm=%.6f stable_adv=%.8f fluct_divisions=%d/8" % [seed, value, stable_adv, int(result["fluctuating_divisions"])])

	var mean_effect: float = _mean(values)
	var median_effect: float = _median(values)
	var mean_stable_adv: float = _mean(stable_values)
	print("M5-C BURDEN-ROBUST summary n=%d mean=%.6f median=%.6f positive=%d stable_valid=%d fluct_divisions=%d/192 mean_stable_adv=%.8f" % [values.size(), mean_effect, median_effect, positive, stable_valid, fluctuating_divisions, mean_stable_adv])

	_assert_true(stable_valid == values.size(), "both architectures divide in every stable-high control seed")
