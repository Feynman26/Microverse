...
# Crucially, each poisson() call consumes exactly one uniform RNG draw regardless
# of lambda or realized event count. The former Knuth loop consumed a variable
# number of uniforms, so a regulatory mutation that changed one transcription
# lambda also phase-shifted the random stream seen by unrelated downstream genes.
# That was a reproducible but nonlocal stochastic artifact, not molecular
# biology. Fixed draw cardinality preserves common-noise pairing and keeps a
# local kinetic change local in the RNG schedule.
#
...
func poisson(lambda_value: float) -> int:
	...
	while u > cumulative:
		...
		# Defensive numerical escape. For supported lambda this should only be
		# reached in the extreme upper tail after cumulative is effectively one.
