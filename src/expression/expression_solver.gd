...
static func step(state, genome, metabolites: Dictionary, dt: float, rng, config) -> Dictionary:
	...
	for gene in genome.genes:
		...
		translation_requests[locus_id] = maxf(0.0, float(config.translation_rate) * float(old_mrna.get(locus_id, 0.0)) * dt * translation_noise)
	...
	metabolites["NUC"] = maxf(0.0, float(metabolites.get("NUC", 0.0)) - effective_transcription * float(config.nuc_cost_per_mrna_unit) + returned_nuc)
	metabolites["AA"] = maxf(0.0, float(metabolites.get("AA", 0.0)) - effective_translation * float(config.aa_cost_per_protein_unit) + returned_aa)
	state.assert_nonnegative()
	return {"transcription": effective_transcription, "translation": effective_translation, "atp_cost": atp_spent, "regulation": regulation}
...
static func _noise_multiplier(rng, fraction: float) -> float:
	# Direct isolated-cell controls from older milestones do not pass the world
	# RNG. They are intentionally deterministic/noise-free; authoritative engine
	# runs always pass the seeded universe RNG.
